...
void AttestationDesktop::DownloadCaptchaImage(
  ...
  const ledger::Result result =
      braveledger_response_util::ParseCaptchaResponse(response, &dictionary);
...
void AttestationDesktop::OnDownloadCaptchaImage(
  ...
  base::Value dictionary(base::Value::Type::DICTIONARY);
  const ledger::Result result = braveledger_response_util::ParseCaptchaResponse(
      captcha_response,
      &dictionary);
  ...
  braveledger_response_util::ParseCaptchaImageResponse(
      response,
      &encoded_image);
...
void AttestationDesktop::OnConfirm(
  ...
  const ledger::Result result =
      braveledger_response_util::ParseConfirmAttestationResponse(response);
  if (result != ledger::Result::LEDGER_OK) {
