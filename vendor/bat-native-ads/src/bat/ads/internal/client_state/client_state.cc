...
Result ClientState::FromJson(
  ...
  if (document.HasMember("userModelLanguage")) {
    user_model_language = document["userModelLanguage"].GetString();
  }

  if (document.HasMember("userModelLanguages")) {
    for (const auto& language : document["userModelLanguages"].GetArray()) {
