...
void RedeemUnblindedToken::Redeem(
  ...
  if (ads_->get_unblinded_tokens()->IsEmpty()) {
    BLOG(1, "No unblinded tokens to redeem");
    return;
  }

  const privacy::UnblindedTokenInfo unblinded_token
      = ads_->get_unblinded_tokens()->GetToken();
  ads_->get_unblinded_tokens()->RemoveToken(unblinded_token);
...
void RedeemUnblindedToken::CreateConfirmation(
  ...
  CreateConfirmationRequest request(ads_);

  auto url = request.BuildUrl(confirmation.id, confirmation.credential);

  auto method = request.GetMethod();

  const auto client_info = ads_->get_ads_client()->GetClientInfo();
...
void RedeemUnblindedToken::OnFetchPaymentToken(
  ...
  privacy::UnblindedTokenInfo unblinded_payment_token_info;
  ...
  if (ads_->get_unblinded_payment_tokens()->TokenExists(
      unblinded_payment_token_info)) {
  ...
  ads_->get_unblinded_payment_tokens()->AddTokens(tokens);
  ...
  BLOG(1, "Added 1 unblinded payment token with an estimated redemption value "
      "of " << estimated_redemption_value << " BAT, you now have "
          << ads_->get_unblinded_payment_tokens()->Count()
...
void RedeemUnblindedToken::CreateAndAppendNewConfirmationToRetryQueue(
  ...
  confirmations_->RefillUnblindedTokensIfNecessary();
}
...
ConfirmationInfo RedeemUnblindedToken::CreateConfirmationInfo(
  ...
  confirmation.token_info = unblinded_token;
...
bool RedeemUnblindedToken::Verify(
