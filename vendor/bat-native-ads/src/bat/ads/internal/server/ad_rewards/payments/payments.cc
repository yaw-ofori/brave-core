...
bool Payments::SetFromDictionary(
  ...
  for (auto& value : list_value) {
    ...
    // Amount
    const base::Optional<double> amount_value =
        dictionary->FindDoubleKey("amount");
    if (!amount_value) {
      return false;
    }

    payment.amount = amount_value.value_or(0.0);
...
base::Value Payments::GetAsList() {
  ...
  for (const auto& payment : payments_) {
    ...
    dictionary.SetKey("amount", base::Value(payment.amount));
...
base::Time Payments::CalculateNextPaymentDate(
  ...
  if (now_exploded.day_of_month <= kNextPaymentDay) {
    ...
    if (HasPendingAmountForTransactionMonth(previous_month)) {
      // If last month has a pending amount, then the next payment date will
      // occur this month
    } else {
      // If last month does not have a pending amount, then the next payment
      // date will occur next month
      month++;
    }
  } else {
    auto this_month = GetTransactionMonth(time);
    if (HasPendingAmountForTransactionMonth(this_month)) {
      // If this month has a pending amount, then the next payment date will
      // occur next month
      month++;
    } else {
      ...
      if (next_token_redemption_date_exploded.month == month) {
        // If this month does not have a pending amount and our next token
...
PaymentList Payments::GetFromList(
  ...
  for (auto& item : *list) {
    ...
    if (!GetAmountFromDictionary(
        dictionary, &payment.amount)) {
      continue;
    }
...
bool Payments::GetAmountFromDictionary(
    base::DictionaryValue* dictionary,
    double* amount) const {
  DCHECK(dictionary);
  DCHECK(amount);
  ...
  if (!base::StringToDouble(amount_value, amount)) {
    return false;
  }
...
bool Payments::HasPendingAmountForTransactionMonth(
