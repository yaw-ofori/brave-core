...
#include "bat/ads/internal/request_signed_tokens_request.h"

#include <string>
#include <vector>

#include "testing/gtest/include/gtest/gtest.h"
#include "bat/ads/internal/privacy_utils.h"
...
TEST(BatAdsRequestSignedTokensRequestBuilderTest,
    BuildUrl) {
  // Arrange
  WalletInfo wallet;
  ...
  const RequestSignedTokensRequestBuilder request_builder;

  // Act
  const std::string url = request_builder.BuildUrl(wallet);

  // Assert
  const std::string expected_url = R"(https://ads-serve.bravesoftware.com/v1/confirmation/token/d4ed0af0-bfa9-464b-abd7-67b29d891b8b)";
...
TEST(BatAdsRequestSignedTokensRequestBuilderTest,
    GetMethod) {
  // Arrange
  const RequestSignedTokensRequestBuilder request;
...
TEST(BatAdsRequestSignedTokensRequestBuilderTest,
  ...
  // Act
  const std::string body = request.BuildBody(blinded_tokens);

  // Assert
  const std::string expected_body = R"({"blindedTokens":["iEK4BXJINfAa0kzgpnnukGUAHvH5303+Y/msR5+u/nY=","eAAv7FNH2twpELsYf3glHLlOhnnlIMovIeEgEmcjgyo=","1G0+8546Y6jCIUXG0cKJq0qpkd6NsnG+4w9oSVW3gH8="]})";
...
TEST(BatAdsRequestSignedTokensRequestBuilderTest,
  ...
  // Act
  const std::vector<std::string> headers = request.BuildHeaders(body, wallet);
...
TEST(BatAdsRequestSignedTokensRequestBuilderTest,
  ...
  // Act
  const std::string header_value = request.BuildDigestHeaderValue(body);
...
TEST(BatAdsRequestSignedTokensRequestBuilderTest,
  ...
  const std::string header_value =
      request.BuildSignatureHeaderValue(body, wallet);

  // Assert
  const std::string expected_header_value = R"(keyId="primary",algorithm="ed25519",headers="digest",signature="m5CxS9uqI7DbZ5UDo51bcLRP2awqcUSU8tfc4t/ysrH47B8OJUG1roQyi6/pjSZj9VJuj296v77c/lxBlCn2DA==")";
...
TEST(BatAdsRequestSignedTokensRequestBuilderTest,
  ...
  // Act
  const std::string accept_header_value = request.GetAcceptHeaderValue();
