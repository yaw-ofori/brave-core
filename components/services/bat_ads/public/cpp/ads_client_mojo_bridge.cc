...
bool AdsClientMojoBridge::GetUserModelPath(
    const std::string& model_id,
    std::string* out_path) {
  DCHECK(out_path);
  *out_path = ads_client_->GetUserModelPath(model_id);
  return true;
}

void AdsClientMojoBridge::GetUserModelPath(
    const std::string& model_name,
    GetUserModelPathCallback callback) {
  std::move(callback).Run(ads_client_->GetUserModelPath(model_name));
}
